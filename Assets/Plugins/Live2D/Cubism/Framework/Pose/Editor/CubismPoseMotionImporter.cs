...
namespace Live2D.Cubism.Editor.Importers
{
    ...
    internal static class CubismPoseMotionImporter
    {
        ...
        private static async UniTask OnModelImport(CubismModel3JsonImporter sender, CubismModel model)
        {
            var shouldImportAsOriginalWorkflow = CubismUnityEditorMenu.ShouldImportAsOriginalWorkflow;
            var pose3Json = await (await sender.GetModel3JsonAsync()).GetPose3JsonAsync();
            ...
            var fileReferences = (await sender.GetModel3JsonAsync()).FileReferences;
