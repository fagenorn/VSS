...
namespace Live2D.Cubism.Framework.Json
{
    ...
    public sealed class CubismModel3Json
    {
        ...
        /// <returns></returns>
        public delegate UniTask<object> LoadAssetAtPathHandler(Type assetType, string assetPath);
        ...
        public delegate UniTask<Texture2D> TexturePicker(CubismModel3Json sender, CubismDrawable drawable);
        ...
        public static async UniTask<CubismModel3Json> LoadAtPathAsync(string assetPath)
        {
            // Use default asset load handler.
            return await LoadAtPathAsync(assetPath, BuiltinLoadAssetAtPathAsync);
        }
        ...
        /// <returns>The <see cref="CubismModel3Json"/> on success; <see langword="null"/> otherwise.</returns>
        public static async UniTask<CubismModel3Json> LoadAtPathAsync(string assetPath, LoadAssetAtPathHandler loadAssetAtPath)
        {
            // Load Json asset.
            var modelJsonAsset = await loadAssetAtPath(typeof(string), assetPath) as string;
        ...
        /// </remarks>
        public UniTask<byte[]> GetMoc3Async()
        {
        ...
        /// </summary>
        public async UniTask<CubismPose3Json> GetPose3JsonAsync()
        {
            if (_pose3Json != null)
            {
                return _pose3Json;
            ...
            var jsonString = string.IsNullOrEmpty(FileReferences.Pose) ? null : await LoadReferencedAsset<String>(FileReferences.Pose);
            _pose3Json = CubismPose3Json.LoadFrom(jsonString);
            return _pose3Json;
        }

        /// <summary>
        /// <see cref="GetExpression3JsonsAsync()"/> backing field.
        /// </summary>
        [NonSerialized]
        ...
        /// </remarks>
        public async UniTask<CubismExp3Json[]> GetExpression3JsonsAsync()
        {
            // Fail silently...
            ...
            if (_expression3Jsons == null)
            {
                ...
                for (var i = 0; i < _expression3Jsons.Length; ++i)
                {
                    var expressionJson = (string.IsNullOrEmpty(FileReferences.Expressions[i].File))
                                            ? null
                                            : await LoadReferencedAsset<string>(FileReferences.Expressions[i].File);
        ...
        /// </summary>
        public async UniTask<string> GetPhysics3JsonAsync()
        {
            return string.IsNullOrEmpty(FileReferences.Physics) ? null : await LoadReferencedAsset<string>(FileReferences.Physics);
        }

        public async UniTask<string> GetUserData3JsonAsync()
        {
            return string.IsNullOrEmpty(FileReferences.UserData) ? null : await LoadReferencedAsset<string>(FileReferences.UserData);
        }

        /// <summary>
        /// The contents of cdi3.json asset.
        /// </summary>
        public async UniTask<string> GetDisplayInfo3JsonAsync()
        {
            return string.IsNullOrEmpty(FileReferences.DisplayInfo) ? null : await LoadReferencedAsset<string>(FileReferences.DisplayInfo);
        }

        /// <summary>
        /// <see cref="GetTextures()"/> backing field.
        /// </summary>
        [NonSerialized]
        ...
        /// The references aren't cached internally.
        /// </remarks>
        public async UniTask<Texture2D[]> GetTexturesAsync()
        {
            ...
            if (_textures == null)
            {
                ...
                {
                    _textures[i] = await LoadReferencedAsset<Texture2D>(FileReferences.Textures[i]);
                }
            }
        ...
        public UniTask<CubismModel> ToModelAsync(bool shouldImportAsOriginalWorkflow = false)
        {
            return ToModelAsync(CubismBuiltinPickers.MaterialPicker, CubismBuiltinPickers.TexturePickerAsync, shouldImportAsOriginalWorkflow);
        }
        ...
        /// <param name="shouldImportAsOriginalWorkflow">Should import as original workflow.</param>
        /// <returns>The instantiated <see cref="CubismModel">model</see> on success; <see langword="null"/> otherwise.</returns>
        public async UniTask<CubismModel> ToModelAsync(MaterialPicker pickMaterial, TexturePicker pickTexture, bool shouldImportAsOriginalWorkflow = false)
        {
            // Initialize model source and instantiate it.
            var mocAsBytes = await GetMoc3Async();
            ...
            model.transform.localPosition = new Vector3(-2, 0);
            ...
            for (var i = 0; i < renderers.Length; ++i)
            {
                renderers[i].MainTexture = await pickTexture(this, drawables[i]);
            }
            ...
            //Load from cdi3.json
            var DisplayInfo3JsonAsString = await GetDisplayInfo3JsonAsync();
            ...
            var physics3JsonAsString = await GetPhysics3JsonAsync();
            ...
            var userData3JsonAsString = await GetUserData3JsonAsync();
        ...
        private async UniTask<T> LoadReferencedAsset<T>(string referencedFile) where T : class
        {
            var assetPath = Path.GetDirectoryName(AssetPath) + "/" + referencedFile;

            return await LoadAssetAtPath(typeof(T), assetPath) as T;
        }
        ...
        /// <returns>The asset on success; <see langword="null"/> otherwise.</returns>
        private static async UniTask<object> BuiltinLoadAssetAtPathAsync(Type assetType, string assetPath)
        {
