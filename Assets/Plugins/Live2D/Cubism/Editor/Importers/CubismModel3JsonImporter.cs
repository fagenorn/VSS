...
namespace Live2D.Cubism.Editor.Importers
{
    ...
    {
        /// <summary>
        /// <see cref="GetModel3JsonAsync()"/> backing field.
        /// </summary>
        ...
        /// </summary>
        public async UniTask<CubismModel3Json> GetModel3JsonAsync()
        {
            if (_model3Json == null)
            {
                _model3Json = await CubismModel3Json.LoadAtPathAsync(AssetPath);
            }
        ...
        /// </summary>
        public override async UniTask ImportAsync()
        {
            var isImporterDirty = false;


            // Instantiate model source and model.
            var model = await (await GetModel3JsonAsync()).ToModelAsync(CubismImporter.OnPickMaterial, CubismImporter.OnPickTexture, ShouldImportAsOriginalWorkflow);
            var moc = model.Moc;
            ...
            if (ModelPrefab == null)
            {
                ...
                foreach (var texture in await (await GetModel3JsonAsync()).GetTexturesAsync())
                {
