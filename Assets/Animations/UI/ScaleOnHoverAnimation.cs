...
public class ScaleOnHoverAnimation : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, IPointerDownHandler
{
    ...
    void Start()
    {
        foreach (var item in _gameObjects)
        {
            ...
            item.GameObject.GetAsyncBecameInvisibleTrigger().Subscribe(unit => StopAnimation());
    ...
    void IPointerEnterHandler.OnPointerEnter(PointerEventData eventData)
    {
        foreach (var item in _gameObjects)
        {
            ...
            {
                AnimationType.Once => DOTween.Sequence().Append(item.Transform.DOBlendableScaleBy(item.ScaleBy, item.Duration))
                                                                               .SetAutoKill(false),
                AnimationType.PulseAndRotate => DOTween.Sequence()
                                                                              ...
                                                                              .Join(item.Transform.DOPunchScale(item.ScaleBy, item.Duration, 3, .5f))
                                                                              .Append(item.Transform.DOBlendableLocalRotateBy((-item.RotateAmount) * 2, item.Duration, RotateMode.FastBeyond360))
